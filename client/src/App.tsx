...
function AppContent() {
  const { isLoggedIn, isLoading } = useAuth();
  const [, pathname] = window.location.href.split(window.location.origin);

  useEffect(() => {
    // Skip settings fetch on the trial page
    const isTrialPage = pathname === '/try' || pathname.startsWith('/try/');
    
    // Only fetch settings if user is logged in, auth state is loaded, and not on trial page
    if (!isLoading && isLoggedIn && !isTrialPage) {
